defmodule Euler.Exercise_078 do
  ...
  def solution() do
    ...
    |> Stream.drop_while(fn {a, b} -> rem(b, 1_000_000) != 0 end)
    |> Enum.take(1)
  end
end
...
defmodule Euler.Partitions do
  ...
  def p_from_cache(n, _cache) when n < 0, do: 0
  def p_from_cache(0, _cache), do: 1
  def p_from_cache(1, _cache), do: 1
  def p_from_cache(n, cache), do: Map.get(cache, n)
end
...
defmodule Euler.Pentagonal do
  ...
  def next(x) when x > 0, do: -1 * x
  def next(x) when x < 0, do: abs(x) + 1
end
